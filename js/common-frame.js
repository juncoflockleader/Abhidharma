...
function activateTab(tabIdOrLegacyIndex, options = {}) {
    ...
    document.body.classList.toggle(
        ...
        entry.id === 'citta' || entry.id === 'rupa' || entry.id === 'sense-flow' || entry.id === 'mind-flow' || entry.id === 'rupa-origin' || entry.id === 'dependent-origin' || entry.id === 'conditions-map' || entry.id === 'cause-condition'
    );
    ...
    window.dispatchEvent(new CustomEvent('abhidharma:tabchange', {detail: activeTab}));
    return activeTab;
}
